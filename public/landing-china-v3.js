      ...
      function fairCardHtml(p) {
        ...
        const phases = p.phases
          ...
          .join("");
        // Misma anatomía que la tarjeta de itinerario: la tarjeta cerrada no
        // lleva CTA de contacto (ahí la intención es ver más), el precio es el
        // ancla y el WhatsApp vive al final del detalle abierto.
        const waDetail = `<a class="btn btn-wa pkg-wa" data-wa data-product="${p.id}" data-journey-type="fair" data-cta-location="fair_detail" href="${waLink(productMessage(p))}" target="_blank" rel="noopener">${WA_ICO}Consultar por WhatsApp</a>`;
        return `<article class="pkg-compact pkg-compact--fair active" data-id="${p.id}"><button class="pkg-compact-select" type="button" data-map-select="${p.id}" aria-pressed="true" aria-label="Mostrar ${p.customerName} en el mapa"><img class="pkg-compact-thumb" src="img/${p.image}.webp" alt="" width="1200" height="800" loading="lazy"><span class="pkg-compact-body"><span class="pkg-compact-name">${p.customerName}</span><span class="pkg-compact-kicker">${p.nights} noches · ${p.days} días · ${p.phases.length} fases</span><span class="pkg-compact-cities">Cantón / Guangzhou</span><span class="pkg-compact-offer"><span class="pkg-compact-price"><span class="pkg-compact-price-prefix">${p.priceLabel}</span><span class="pkg-compact-price-amount">${money(p.priceUsd)}</span></span><span class="pkg-compact-basis">Por persona y ${p.priceUnit}, impuestos incluidos</span><span class="pkg-flag">Sin aéreo internacional</span></span></span></button><div class="pkg-compact-actions"><button class="pkg-compact-info" type="button" data-toggle="${p.id}" aria-label="Ver detalle de ${p.customerName}" aria-expanded="false" aria-controls="${detailId}"><span class="pkg-compact-info-label">Ver detalle</span><span class="pkg-compact-info-chevron" aria-hidden="true">⌄</span></button></div><div class="pkg-compact-detail pkg-detail" id="${detailId}" aria-hidden="true" hidden><div class="detail-inner"><div class="pkg-mini-map route-map-frame" data-mini-map="${p.id}" aria-hidden="true"></div><div class="detail-grid"><p><b>Sede:</b> Cantón / Guangzhou (${p.nights}n) · ${p.days} días</p><p><b>Por qué elegirlo:</b> ${p.hook}</p></div><div class="fair-phases"><b>Fases de la Edición 140:</b><ul>${phases}</ul></div><div class="highlights">${p.highlights.map((x) => `<span><b class="hl-check">✓</b> ${x}</span>`).join("")}</div><p><b>Acompañamiento:</b> ${p.accompaniment}</p><p><b>A tener en cuenta:</b> ${p.particular}</p><p class="fair-excludes"><b>No incluye:</b> ${p.excludes}. El vuelo desde Argentina se cotiza por separado.</p><p class="common">${p.includes}</p><div class="pkg-compact-cta"><p class="pkg-compact-cta-title">¿Te interesa esta edición?</p><p class="pkg-compact-cta-lead">Te resolvemos la logística de la fase que necesites.</p>${waDetail}<p class="pkg-compact-cta-note">Sin compromiso.</p></div></div></div></article>`;
      }
